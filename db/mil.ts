...
function getLandscapeImages(): MilMetadata[] {
  const allMilImages = parseMilJson();
  return allMilImages.filter((mil) => mil.orientation === "landscape");
}

function getPortraitImages(): MilMetadata[] {
  const allMilImages = parseMilJson();
  return allMilImages.filter((mil) => mil.orientation === "portrait");
}
